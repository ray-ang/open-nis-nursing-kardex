...
function rja_single_room_content_header()
{
	...
	if ( isset($_POST['edit-room']) && wp_verify_nonce($_POST['token'], 'token') && ( current_user_can('administrator') || current_user_can('nurse') ) ) {	
		...
		update_metadata( 'post', $pid, 'room_last_edit_user', Basic::encrypt($current_user->display_name) );
		update_metadata( 'post', $pid, 'room_last_edit_date', Basic::encrypt(date("Y-m-d")) );
		update_metadata( 'post', $pid, 'room_last_edit_time', Basic::encrypt(date("h:i:sa")) );
		...
		update_metadata( 'post', $pid, 'room_diagnostics', Basic::encrypt($_POST['diagnostics']) );
	...
	if ( get_post_type() == 'room' ) {
		...
		if ( current_user_can('administrator') || current_user_can('nurse') ) {
			...
			<div id="kardex">
				...
				<small style="display: block; clear: both;">Kardex report generated by <?php $current_user = wp_get_current_user(); echo $current_user->display_name; ?> on <?php echo date("Y-m-d"); ?> at <?php echo date("h:i:sa"); ?>.</small>
				<div id="no-print">
